...
export default function CartReview() {
  ...
  const kindOf = (it) => {
    ...
    const type = (it?.meta?.type ?? it?.type ?? "").toLowerCase(); // "company" | "individual"
    if (mode === "gift") return "gift";
              ...
              >
                ...
                {kind === "company" && it.csr && (
                  <div className="mt-1 text-white/70">
                    {t("cart.csrLine", { plan: it.csr.title, price: (it.csr.price ?? 0).toLocaleString(lang === "fr" ? "fr-FR" : "en-US") })}
                  </div>
                )}

                {/* Gift-only add-ons (chips) */}
                ...
                <div className="mt-3 flex items-center justify-between border-t border-white/10 pt-3">
                  ...
                  <div className="text-base font-semibold text-white">€{(it.total ?? 0).toLocaleString(lang === "fr" ? "fr-FR" : "en-US")}</div>
                </div>
              </div>
            );
          })}
            ...
            <div className="fixed inset-0 z-50 grid place-items-center bg-black/60">
              <div className="w-[min(720px,92vw)] rounded-2xl bg-white p-6 text-emerald-950 shadow-2xl">
                <div className="flex items-start justify-between">
                  ...
                  <button onClick={() => setDetailsItem(null)} className="text-emerald-700 hover:text-emerald-900" aria-label={t("cart.details.close")}>×</button>
                </div>
